...
def _GetMarginTop(image):
    width, height = image.size
    vslice = image[width / 2:width / 2 + 1, 0:height / 3]
    black = Color('#000')
...
class Map(Rectangle):
    ...
    def _RefreshCoordinates(self):
        ...
        margin_top = _GetMarginTop(self._map_image)
        if self._margin_top == margin_top:
            return
    ...
    def _GetMapGeometry(self):
        ...
        height = self.HEIGHT - self._margin_top - self.MARGIN_BOTTOM

        return self.MARGIN_LEFT, self._margin_top, width, height
